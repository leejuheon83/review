...
**상세 가이드**: [docs/vercel-firebase-setup.md](docs/vercel-firebase-setup.md)

- Firestore 또는 Realtime Database를 영구 저장소로 사용
- 설정이 없으면 메모리 시드 DB로 동작
- **Vercel 배포 시** 반드시 환경변수 등록 필요 (데이터 영속성)
...
### Vercel 환경변수 (필수 - 삭제/저장 등 데이터 영속성)

[Firebase + Vercel 연동 가이드](docs/vercel-firebase-setup.md) 참고. `.env.example` 변수 목록을 Vercel `Settings > Environment Variables`에 등록하세요.
